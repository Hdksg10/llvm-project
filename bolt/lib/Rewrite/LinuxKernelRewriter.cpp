  ...
  for (BinaryFunction &BF : llvm::make_second_range(BC.getBinaryFunctions())) {
    ...
    for (BinaryBasicBlock &BB : BF) {
      for (MCInst &Inst : BB) {
        ...
        BugTableSection->addRelocation(EntryOffset, Label, Relocation::getPC32(),
                                       /*Addend*/ 0);
      }
    }
    ...
    for (const uint32_t ID : FunctionBugList[&BF]) {
      if (!EmittedIDs.count(ID)) {
        const uint64_t EntryOffset = (ID - 1) * BUG_TABLE_ENTRY_SIZE;
        BugTableSection->addRelocation(EntryOffset, nullptr, Relocation::getPC32(),
                                       /*Addend*/ 0);
      }
    }
  }
...
Error LinuxKernelRewriter::readStaticKeysJumpTable() {
  ...
  while (Cursor && Cursor.tell() < Stop->getAddress() - SectionAddress) {
    ...
    if (!BF || !BC.shouldEmit(*BF))
      continue;
    assert(BF->getOriginSection() && "the function did not originate from the file");
    Info.BF = BF;
