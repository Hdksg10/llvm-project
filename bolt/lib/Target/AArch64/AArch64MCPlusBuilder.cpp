...
public:
  ...
  bool isUBSanitizerTrap(const MCInst &Inst) const override {
    if (Inst.getOpcode() != AArch64::BRK || Inst.getNumOperands() != 1 ||
        !Inst.getOperand(0).isImm())
      return false;

    constexpr int64_t SanitizerTrapMask = 0xFF00;
    constexpr int64_t SanitizerTrapBase = 0x5500;
    constexpr int64_t SanitizerHandlerMask = 0x00FF;
    const int64_t Immediate = Inst.getOperand(0).getImm();
    if ((Immediate & SanitizerTrapMask) != SanitizerTrapBase)
      return false;

    switch (Immediate & SanitizerHandlerMask) {
    case 0x00:
    case 0x01:
    case 0x02:
    case 0x03:
